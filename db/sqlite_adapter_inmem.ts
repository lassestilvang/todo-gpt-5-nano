...
export default class InMemorySqliteAdapter {
  ...
  constructor(seedFromJson: boolean = true) {
    ...
    if (seedFromJson) {
      // Attempt to seed from existing planner.json in data (best-effort)
      try {
        const raw = require('../data/planner.json')
        // Basic seed: if Planner.json has lists/tasks arrays, push into in-memory store
        if (raw?.lists) {
